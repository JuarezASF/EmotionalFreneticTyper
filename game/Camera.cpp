...
unsigned Camera::BACKGROUND_VIEW = 0;
unsigned Camera::PLAYER_GROUND_VIEW = 1;
unsigned Camera::TOPGROUND_VIEW = 2;
...
Vec2 Camera::getPos(unsigned int k) {
    ...
    return paralax_pos[k]; //+ Vec2(((StageState&) Game::getInstance().getCurrentState()).getPanel().GetLeftWidth(),0);
}
